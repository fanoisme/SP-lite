...
export function usePromoRule() {
  ...
  const exportColumns = [
    ...
    { key: 'prm_discount_type', label: 'PRIME Discount Type' },
    { key: 'prm_discount_value', label: 'PRIME Discount Value', format: (v, row) => row.prm_discount_type === 'PERCENTAGE' ? v + '%' : v },
    { key: 'prm_max_discount', label: 'PRIME Max Discount', format: v => Number(v) >= 49999999999 ? 'Unlimited' : v },
    { key: 'pl_discount_type', label: 'PAYLATER Discount Type' },
    { key: 'pl_discount_value', label: 'PAYLATER Discount Value', format: (v, row) => row.pl_discount_type === 'PERCENTAGE' ? v + '%' : v },
    { key: 'pl_max_discount', label: 'PAYLATER Max Discount', format: v => Number(v) >= 49999999999 ? 'Unlimited' : v },
