...
export function usePromoRule() {
  ...
  const exportColumns = [
    ...
    { key: 'prm_max_discount', label: 'PRIME Max Discount', format: (v, row) => !row.prm_discount_type ? null : (Number(v) >= 49999999999 ? 'Unlimited' : v) },
    ...
    { key: 'pl_max_discount', label: 'PAYLATER Max Discount', format: (v, row) => !row.pl_discount_type ? null : (Number(v) >= 49999999999 ? 'Unlimited' : v) },
  ...
  async function exportFiltered() {
    try {
      let query = supabase.from('qrdd_promo_rules').select('*').order('updated_at', { ascending: false })
