<template>
  <button 
    :class="['li-btn', `li-btn-${variant}`, `li-btn-${size}`, { 'li-btn-icon-only': isIconOnly, 'li-btn-loading': loading }]"
    :disabled="disabled || loading"
    @click="handleClick"
    ref="btn"
  >
    <span v-if="loading" class="li-btn-spinner"></span>
    <span v-if="iconLeft && !loading" class="li-btn-icon li-btn-icon-left">
      <slot name="iconLeft"><component :is="iconLeft" /></slot>
    </span>
    <span class="li-btn-label" :class="{ 'sr-only': isIconOnly }">
      <slot></slot>
    </span>
    <span v-if="iconRight && !loading" class="li-btn-icon li-btn-icon-right">
      <slot name="iconRight"><component :is="iconRight" /></slot>
    </span>
  </button>
</template>

<script setup>
import { ref, computed } from 'vue';
import { useRipple } from '../composables/useRipple';

const props = defineProps({
  variant: {
    type: String,
    default: 'primary',
    validator: (v) => ['primary', 'secondary', 'danger', 'ghost'].includes(v)
  },
  size: {
    type: String,
    default: 'md',
    validator: (v) => ['sm', 'md', 'lg'].includes(v)
  },
  iconLeft: [Object, String],
  iconRight: [Object, String],
  isIconOnly: Boolean,
  disabled: Boolean,
  loading: Boolean,
});

const emit = defineEmits(['click']);
const btn = ref(null);
const { addRipple } = useRipple();

const handleClick = (e) => {
  if (props.disabled || props.loading) return;
  addRipple(e, btn.value);
  emit('click', e);
};
</script>

<style scoped>
.li-btn {
  position: relative;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: var(--space-s, 8px);
  border: none;
  cursor: pointer;
  border-radius: var(--radius-pill, 999px);
  font-family: var(--font-family, 'Inter', sans-serif);
  font-weight: 600;
  transition: all var(--dur-short, 200ms) var(--ease-out, cubic-bezier(0.4, 0, 0.2, 1));
  overflow: hidden;
  user-select: none;
  outline: none;
}

.li-btn:focus-visible {
  box-shadow: 0 0 0 2px var(--color-gray-0, #fff), 0 0 0 4px var(--color-orange-400, #FF6B00);
}

.li-btn:active {
  transform: translateY(1px);
}

.li-btn:disabled, .li-btn-loading {
  cursor: not-allowed;
  transform: none;
}

/* Sizes */
.li-btn-sm {
  padding: 6px 12px;
  font-size: var(--text-xs, 14px);
  height: 32px;
}

.li-btn-md {
  padding: 8px 16px;
  font-size: var(--text-sm, 16px);
  height: 40px;
}

.li-btn-lg {
  padding: 12px 24px;
  font-size: var(--text-sm, 16px);
  height: 48px;
}

.li-btn-icon-only {
  padding: 0;
  width: auto;
  aspect-ratio: 1/1;
}

/* Variants */
.li-btn-primary {
  background-color: var(--cta-primary-bg, #FFBC25);
  color: var(--cta-primary-text, #1E1E1E);
}
.li-btn-primary:hover:not(:disabled) {
  background-color: var(--cta-primary-hover, #FAB000);
}

.li-btn-secondary {
  background-color: transparent;
  color: var(--cta-secondary-text, #333333);
  border: 1.5px solid var(--cta-secondary-border, #CCCCCC);
}
.li-btn-secondary:hover:not(:disabled) {
  border-color: var(--color-orange-400, #FF6B00);
}

.li-btn-danger {
  background-color: var(--cta-danger-bg, #C83E3B);
  color: var(--cta-danger-text, #FFFFFF);
}
.li-btn-danger:hover:not(:disabled) {
  background-color: var(--color-red-500, #A33129);
}

.li-btn-ghost {
  background-color: transparent;
  color: var(--cta-ghost-text, #808080);
}
.li-btn-ghost:hover:not(:disabled) {
  background-color: var(--cta-ghost-hover-bg, #F2F2F2);
  color: var(--color-gray-900, #333333);
}

/* Disabled overrides */
.li-btn:disabled {
  background-color: var(--cta-disabled-bg, #EDF0F2);
  color: var(--cta-disabled-text, #999999);
  border-color: transparent;
}

/* Utils */
.sr-only {
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
  margin: -1px;
  overflow: hidden;
  clip: rect(0, 0, 0, 0);
  border: 0;
}

.li-btn-spinner {
  width: 16px;
  height: 16px;
  border: 2px solid currentColor;
  border-bottom-color: transparent;
  border-radius: 50%;
  animation: spin 1s linear infinite;
}

@keyframes spin {
  100% { transform: rotate(360deg); }
}

:deep(.li-ripple-animation) {
  position: absolute;
  border-radius: 50%;
  transform: scale(0);
  animation: ripple var(--dur-medium, 300ms) linear;
  background-color: rgba(255, 255, 255, 0.4);
  pointer-events: none;
}

.li-btn-secondary :deep(.li-ripple-animation),
.li-btn-ghost :deep(.li-ripple-animation) {
  background-color: rgba(0, 0, 0, 0.1);
}

@keyframes ripple {
  to {
    transform: scale(4);
    opacity: 0;
  }
}
</style>
